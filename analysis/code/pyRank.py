...
def clean_database(args):
    ...
    if args.clean:
        ...
        #output = add_followers(clean_connection, clean)  # Re-enable for more precision
        return edges
    else:
        while True:
...
def visualize(edges):
    ...
    position = networkx.spring_layout(graph, k=2/51)
    networkx.draw_networkx_nodes(graph, position, node_size=25)
