...
power_method <- function(rankings, A, iterations, users) {
    q = list()
    q[[1]] <- A
    for (i in 2:iterations) {
        q[[i]] <- rankings[,] %*% q[[i-1]]
    }
    write.table(q[[iterations]], file="eigenvector.txt")
    q
}
